...
to add credit or create charges against the `Account`. Invoices can also be automatically generated by the system when
...
This endpoint is used to insert existing invoices in the use case of a migration scenario from either a third party system or an in-house billing system into Kill Bill.
...
Create charges for a given `Account`. This will result in the creation of a new `Invoice`.
...
The normal scenario to add `TAX` items is to rely on a (tax) invoice plugin, intercepting the `Invoice` during its creation and adding on the fly
the required `TAX` items. In some scenario, we also want to add `TAX` items directly. This will result in the creation of a new `Invoice`.
...
| Name | Type | Required | Description |
| ---- | -----| -------- | ----------- | 
| **externalPayment** | boolean | false | choose if the payment method should be defaulted to the (system provided) external payment method. Default is false |
...
Custom fields are `{key, value}` attributes that can be attached to any customer resources, and in particularly on the `Invoice` objects.
