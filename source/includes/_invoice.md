...
there are existing active subscriptions for this `Account`. In the later case, the `targetDate` will determine up to which point to invoice,
...
when computing the account balance, overdue status, ... A `COMMITTED` invoice becomes immutable, and its balance is reflected at the `Account` level.
...
The following invoice items types are currently supported:
...
* `REPAIR_ADJ`: Internal adjustment, used for billing in advance and subscription state changed
...
* `USAGE`: Usage item
* `TAX`: Tax item -- this can only be added through a plugin
