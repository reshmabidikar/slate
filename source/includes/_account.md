...
Each of your customers is identified by an account. The information for an account is contained in an account resource. The endpoints in this group manage the account resource and the information it contains. 
...
| **billCycleDayLocal** | 0 |
| **isPaymentDelegatedToParent** | false |
| **referenceTime** | time-date code for the current time in the specified timezone |
| **timeZone** | "UTC" |
| all others | null |
...
A `201` status is returned if the call succeeds, along with a `Location` header to indicate how to fetch the resource.
